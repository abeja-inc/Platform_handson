...
with open('./scripts/credential.json', 'r') as f:
...
def register_dataset_items_from_datalake(organization_id, channel_id, dataset_name, split, max_workers):
    """
    ...
    with open('./scripts/dataset.json', 'r') as f:
        dataset_props = json.load(f)

    bccd_dataset = bccd_bbox_dataset.VOCBboxDataset(split=split)
    nb_data = len(bccd_dataset)

    data = {}
    for i in range(nb_data):
        id, annotation = bccd_dataset.get_annotations(i)
        data[id] = annotation
...
if __name__ == '__main__':
    ...
    register_dataset_items_from_datalake(organization_id, channel_id, dataset_name, split, max_workers)
